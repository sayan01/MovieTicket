...
public class MovieTicket implements ActionListener {
	...
	public static void main(String[] args) throws IOException {
		if(args.length>0){           // If an argument is passed
			for (String arg : args) {
				if (arg.equalsIgnoreCase("local")) { //If any argument is 'local'
					Database.update = false; // Run locally
				}
			}
		}
	...
	private void sc1()  {
		...
		next.setBounds(width - 95 , 180, 75, 25);// Set the boundaries of next button
	...
	private void sc2(){
		...
		if(movie.length()>50)   movie = movie.substring(0,50)+"...";  // truncate name if big
		...
		date = new JComboBox<>(DateTime.getDates(5));// Setting 5 days from current date
	...
	private void sc3() {
		...
		for(int i = 0; i < seats.length;i++){
			for(int j = 0; j < seats[i].length;j++){
				...
				seat.setToolTipText("Rs. "+Database.price(seatCode));//Show the price of seat
				f.add(seat);   // Add seat to frame
				seats[i][j] = seat; // Store memory address of seat in array
			}
		}
		// For every entry in seats database, check if any entry is of the current show 
	...
	private void sc4() throws IOException{
		...
		// add the current booking to database
		db.setSeats(movie, lan, dat, tim, name, seats_booked); 
	}
	/*
	 ...
	 *   Returns: void
	 *   Description: 
	 *   This method is invoked whenever any button is pressed or any date is selected.
	 *   This method determines what to do when something is clicked.
	 */
	public void actionPerformed(ActionEvent ae) {
		/*
		   If a date is selected, change the timing options accordingly.
		   Eg. if current date is selected,
		   then only show timings which are still in the future.
		   */
		...
		if (ae.getSource() == next) {  // if button is pressed
			...
			if(scene==1){
				...
				// i.e. user has not selected something,
				// then displaying error dialog
			...
			// If next button is pressed and user went to a new scene,
			// then revalidate the elements of the new scene
			f.revalidate();
			f.repaint();
		 ...
		 *
		 */
		if(seats[0][0]==null) return; // if seats are not yet initialized,
			...
			for(int j = 0;j<seats[i].length;j++){
				...
				if(ae.getSource() == seat){
					if(seat.getIcon()==seatIcons[0]) {  // if seat was not selected
						...
						seats_booked.add(""+(char)(j+65)+(i+1));// add the seat to the array
					}
					else if(seat.getIcon()==seatIcons[1]) { //if seat was selected
						seat.setIcon(seatIcons[0]);         //deselect seat
						seats_booked.remove(""+(char)(j+65)+(i+1));//remove seat from array
					}
				}
			}
		}
